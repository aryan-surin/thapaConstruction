    ...
    <div class="container-custom max-w-5xl">
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-primary">Edit Blog Post</h1>
            <p class="text-neutral">Update the blog article details below</p>
          </div>
        </div>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="bg-white rounded-lg shadow-md p-20 text-center">
        <Icon name="svg-spinners:270-ring" class="w-12 h-12 text-accent mx-auto mb-4" />
        <p class="text-neutral">Loading blog post...</p>
      </div>

      <!-- Not Found State -->
      <div v-else-if="!blog" class="bg-white rounded-lg shadow-md p-20 text-center">
        <Icon name="heroicons:exclamation-triangle" class="w-16 h-16 text-red-500 mx-auto mb-4" />
        <h2 class="text-2xl font-bold text-primary mb-2">Blog Not Found</h2>
        <p class="text-neutral mb-6">The blog post you're looking for doesn't exist.</p>
        <NuxtLink to="/admin/blog" class="btn-primary">
      ...
      >
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-8 transform transition-all">
          <div class="text-center">
            ...
            <h3 class="text-2xl font-bold text-primary mb-2">Blog Updated Successfully!</h3>
...
/**
 * Blog Edit Form - Admin
 * 
 * Edit existing blog posts with:
...
// SEO
useHead({
  title: 'Edit Blog Post - Admin | Thapa Construction',
  meta: [
...
const handleSubmit = async () => {
  if (!isFormValid.value || !blog.value) return;
  ...
  try {
    ...
    // Update blog post
    const updatedBlog = blogStore.updateBlog(blog.value.id, formData);
...
onMounted(() => {
  const foundBlog = blogStore.getBlogBySlug(blogId) || blogStore.allBlogs.find((b: BlogPost) => b.id === blogId);
  
  if (foundBlog) {
    blog.value = foundBlog;
    
    // Populate form with existing data
    formData.title = foundBlog.title;
    formData.description = foundBlog.description;
    formData.coverImage = foundBlog.coverImage;
    formData.content = foundBlog.content;
    formData.category = foundBlog.category;
    formData.featured = foundBlog.featured || false;
  }
